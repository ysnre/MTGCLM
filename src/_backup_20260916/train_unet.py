...
def run_experiment(name: str, train_loader, val_loader, epochs: int = EPOCHS, limit_batches: int = None):
    print(f"\n==========================================")
    ...
    if name == "CloudUNet_Fusion":
        model = CloudUNet_Fusion(in_channels=17, scalar_features=3, num_classes=2).to(DEVICE)
    else:
        model = MTGUNet(in_channels=17, num_classes=2).to(DEVICE)
...
def main():
    experiments = {
        "CloudUNet_Fusion": "CloudUNet_Fusion"
    }
    ...
    json_path = os.path.join("artifacts", "unet_results.json")
    results = {}
    ...
    epochs_to_run = 2 if not IS_COLAB else 15
    limit_batches = 20 if not IS_COLAB else None
