...
def train_one_epoch(model, dataloader, criterion, optimizer, device, limit_batches=None):
    ...
    if is_h5 and (limit_batches is None or limit_batches > 100):
...
def run_experiment(name: str, conv_blocks: list, train_loader, val_loader, epochs: int = EPOCHS, limit_batches: int = None, use_gap: bool = True, model_type: str = "ConvNet"):
    """
    ...
    if model_type == "ResNet18":
        ...
        model = MTGResNet18(in_channels=17, num_classes=2).to(DEVICE)
    elif model_type == "ResNet18_LateFusion":
        from model import ResNet18_LateFusion
        model = ResNet18_LateFusion(in_channels=17, scalar_features=3, num_classes=2).to(DEVICE)
    elif model_type == "MTGConvNet_LateFusion":
        from model import MTGConvNet_LateFusion
        model = MTGConvNet_LateFusion(in_channels=17, scalar_features=3, num_classes=2, conv_blocks=conv_blocks, dropout_rate=0.2).to(DEVICE)
    else:
        from model import MTGConvNet
        model = MTGConvNet(in_channels=17, num_classes=2, conv_blocks=conv_blocks, use_gap=use_gap).to(DEVICE)
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        val_loss, val_acc, val_prec, val_rec, val_f1 = validate(model, val_loader, criterion, DEVICE, limit_batches)
    ...
    print(f"Finished {name} in {elapsed/60:.2f} minutes. Best Val F1: {best_f1:.4f}")
    ...
    return history, best_f1, num_params
...
def main():
    ...
    epochs_to_run = 3 if not IS_COLAB else 15
    limit_batches = 30 if not IS_COLAB else None
    ...
    for name, config in experiments.items():
        ...
        history, best_f1, num_params = run_experiment(
            name, 
            blocks, 
            train_loader,
            val_loader,
    ...
    print("="*50)
    print(f"{'Model Name':<12} | {'Params':<10} | {'Best Val F1':<12}")
    print("-"*50)
    for name, res in results.items():
        print(f"{name:<12} | {res['num_params']:<10,} | {res['best_f1']:<12.4f}")
