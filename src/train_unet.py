...
_AUX_NORM = None   # (n_base, mean, std) — f16 modunda aux z-skoru GPU'da (bkz. train.py)
...
def main():
    ...
    parser.add_argument("--no_f16", action="store_true",
                        help="Eski veri yolu (float32 cevirme + aux z-skoru CPU'da). Daha yavas, sonuc ayni.")
    ...
    import json
    json_path = args.results or out_path("unet_results.json")
    results = {}
    ...
    for name, model_type in experiments.items():
        ...
        try:
            ...
            print(f"Uyarı: sonuç yazılamadı: {e}")
